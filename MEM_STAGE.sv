...
module dmem_dual_ip (
    ...
    initial begin
        ...
        // Nếu cần nạp data ban đầu thì dùng đường dẫn tương đối
        // $readmemh("dmem_4b.hex", dmem);
    end

    always_ff @(posedge clock) begin
        if (wren_a) begin
            dmem[address_a[8:0]] <= data_a;
        end

        if (wren_b) begin
            dmem[address_b[8:0]] <= data_b;
        end
    end

    assign q_a = dmem[address_a[8:0]];
    assign q_b = dmem[address_b[8:0]];
